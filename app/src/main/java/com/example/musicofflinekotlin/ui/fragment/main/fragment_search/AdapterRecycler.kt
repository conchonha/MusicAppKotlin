...
class AdapterRecycler : RecyclerView.Adapter<AdapterRecycler.ViewHolder>() {
    ...
    fun setupData(songList: List<Song>,onItemClickSongListener: OnItemClickSongListener,checkHistory: Boolean){
        mListSong = songList
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        var view : View = View.inflate(parent.context,R.layout.item_list_music_fragment_search_history,null)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        Picasso.get().load(Uri.parse(song.mPathUriImage)).error(R.drawable.img_music_error).into(holder.mImgAlbum)

        if(!mCheckHistory){
            holder.mImgClose.visibility = GONE
        }
        holder.mRelativeBox.setOnClickListener {
            mOnItemClickSongListener!!.clickOpenItem(mListSong,position)
        }
        holder.mImgClose.setOnClickListener { mOnItemClickSongListener!!.clickDeleteItem(mListSong,position) }
    }
